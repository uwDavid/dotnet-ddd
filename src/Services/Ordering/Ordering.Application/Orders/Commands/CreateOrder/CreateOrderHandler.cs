    ...
    : ICommandHandler<CreateOrderCommand, CreateOrderResult>
{
    ...
    {
        //1 create Order entity from command obj
        var order = CreateNewOrder(command.Order);

        //2 save to db
        // use IDbContext => to save data to db
        // => so we define a db interface
        // b/c Application layer doesn't reference Infrastructure layer
